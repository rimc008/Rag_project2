...
export default function Heropage({setQuizQuestions1}) {
    ...
    <div className={`min-h-screen font-Prata antialiased transition-colors duration-300 ${
      ...
      <nav className={`flex justify-between items-center px-6 md:px-12 py-5 border-b transition-colors ${
        ...
        <h1 className="text-xl font-bold tracking-tight">
          RAGenius<span className={darkMode ? "text-zinc-400 font-normal" : "text-zinc-500 font-normal"}>AI</span>
        </h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <Button 
            ...
            className={`flex items-center gap-2 rounded-xl transition-all ${
              darkMode 
                ? "border-zinc-800 bg-zinc-900 hover:bg-zinc-800 text-zinc-200 hover:text-white" 
                : "border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-800"
            }`}
          >
            <UploadCloud size={16} />

            <label htmlFor="upload pdf"> {(documentname !== "") ? documentname :"Upload Pdf"} </label>
      ...
      <header className="max-w-4xl mx-auto text-center mt-16 px-6">
        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium border tracking-wide ${
          ...
          <FileText size={12} /> AI Powered PDF Workspace
        </div>
      ...
      <main className="max-w-6xl mx-auto mt-12 grid md:grid-cols-5 gap-8 px-6 pb-24">
        ...
        <section className="md:col-span-2 space-y-6">
          <div className={`border rounded-2xl p-6 shadow-sm transition-all ${
            darkMode ? "bg-zinc-900/40 border-zinc-900" : "bg-zinc-50/50 border-zinc-200"
          }`}>
            ...
            <Button className={`w-full mt-6 rounded-xl py-5 font-medium transition-all ${
              darkMode 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className={`p-4 rounded-xl border ${darkMode ? "bg-zinc-900/20 border-zinc-900" : "bg-zinc-50/30 border-zinc-100"}`}>
              <h3 className="text-sm font-bold">Smart Notes</h3>
              <p className={`text-xs mt-1 ${darkMode ? "text-zinc-500" : "text-zinc-400"}`}>Instantly extract clean, outlines.</p>
            </div>
            <div className={`p-4 rounded-xl border ${darkMode ? "bg-zinc-900/20 border-zinc-900" : "bg-zinc-50/30 border-zinc-100"}`}>
              <h3 className="text-sm font-bold">AI Summary</h3>
              <p className={`text-xs mt-1 ${darkMode ? "text-zinc-500" : "text-zinc-400"}`}>Condense dense sections fast.</p>
            </div>
          </div>
        </section>
        ...
        <section className={`md:col-span-3 border rounded-2xl flex flex-col min-h-[420px] shadow-sm overflow-hidden ${
          darkMode ? "bg-zinc-900/30 border-zinc-900" : "bg-zinc-50/30 border-zinc-200"
        }`}>
          {/* Chat Header */}
          <div className={`px-4 py-3 border-b flex items-center justify-between ${
            darkMode ? "border-zinc-900 bg-zinc-900/50" : "border-zinc-200/60 bg-zinc-100/40"
          }`}>
            <span className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Contextual Assistant</span>
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
          </div>

          {/* Message Stream */}
          <div className="flex-1 p-4 space-y-4 overflow-y-auto text-sm max-h-[320px]">
